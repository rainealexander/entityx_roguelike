...
struct Body {
    Body(const sf::Vector2f& position, const sf::Vector2f& direction,float age, float rotationd = 0.0)
        : position(position), direction(direction), rotationd(rotationd), alpha(0.0), age(age) {}
...
public:
    ...
    void update(ex::EntityManager& es, ex::EventManager& events, ex::TimeDelta dt) override {
        ...
        for (int i = 0; i < count - c; i++) {
            ...
            // "Physical" attributes.
            entity.assign<Body>(
                sf::Vector2f(r(500.0f, 20.0f), r(500.0f, 20.0f)),
                sf::Vector2f(r(200, -50), r(200, -50)),
...
struct BodySystem : public ex::System<BodySystem> {
    void update(ex::EntityManager& es, ex::EventManager& events, ex::TimeDelta dt) override {
        ...
        es.each<Body>([fdt](ex::Entity entity, Body& body) {
            ...
            body.age += fdt;
            std::cout << "body age: " << body.age << std::endl;
            });
    };
};
...
public:
    ...
    void emit_particles(ex::EntityManager& es, ex::Entity entity) {
        ...
        float area = (M_PI * collideable->radius * collideable->radius) / 4.0;
...
class Render_System :public ex::System<Render_System> {
public:
    explicit Render_System(sf::RenderTarget& target, sf::Font& font) : target(target) {
        text.setFont(font);
        text.setPosition(sf::Vector2f(rand() % target.getSize().x, rand() % target.getSize().y));
